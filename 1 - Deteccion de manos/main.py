...
#video = cv2.VideoCapture(0) # Usando la cámara web
video = cv2.VideoCapture("Video.mp4") # Usando un video
mano = mp.solutions.hands
#Mano = mano.Hands(max_num_hands=1)
Mano = mano.Hands(max_num_hands=2)
...
while True:
    ...
    puntos = []
    if handPoints:
        contadores = []  # Lista para almacenar contadores de cada mano
        
        for hand_idx, points in enumerate(handPoints):
        ...
        # Contar dedos para esta mano específica
        ...
        # Mostrar contadores para cada mano
        for i, contador in enumerate(contadores):
            x_pos = 80 + (i * 150)  # Posicionar rectángulos lado a lado
